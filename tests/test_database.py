from atoffice_shell.database import _create_table, get_all_todos, get_db_connection, insert_todo
from atoffice_shell.model import Todo


def test_get_db_connection_returns_fresh_connection(monkeypatch, tmp_path) -> None:
    db_path = tmp_path / "todos.db"
    monkeypatch.setattr("atoffice_shell.database.DB_PATH", db_path)
